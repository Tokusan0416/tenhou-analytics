...
def render_kpi_metrics(stats, game_stats, prev_stats, prev_game_stats):
    ...
    _m(
        ...
        "d",
        pg["total_games"] if pg else None,
        False,
    )
    _m(
        c[1],
        "合計pt",
        game_stats["total_point"],
        "+.1f",
        pg["total_point"] if pg else None,
        False,
    )
    _m(
        c[2],
        "平均pt",
        game_stats["avg_point"],
        "+.1f",
        pg["avg_point"] if pg else None,
        False,
    )
    _m(
        c[3],
    ...
    _m(
        c[5],
        "連対率",
        game_stats["rentai_rate"],
        ".2f",
        pg["rentai_rate"] if pg else None,
        False,
    ...
    _m(
        c[6],
        "ラス率",
        game_stats["last_rate"],
        ".2f",
        pg["last_rate"] if pg else None,
        True,
    ...
    _m(
        ...
        stats["avg_agari_ten"],
        ",.0f",
        ps["avg_agari_ten"] if ps else None,
        False,
    )
    _m(
        c[2],
        ...
        stats["houjuu_rate"],
        ".2f",
        ps["houjuu_rate"] if ps else None,
        True,
    ...
    _m(
        c[3],
        "放銃打点",
        stats["avg_houjuu_ten"],
        ",.0f",
        ps["avg_houjuu_ten"] if ps else None,
        True,
    )
    _m(
        c[4],
...
def render_radar_chart(stats, all_rounds):
    ...
    avg = {
        ...
        "at": (others.loc[others["is_agari"], "agari_ten"].mean() / 1000)
        if others["is_agari"].any()
        else 5,
    ...
    my = [
        n(stats["agari_rate"], *R[0]),
        n(stats["avg_agari_ten"] / 1000, *R[1]),
        n(stats["reach_rate"], *R[2]),
        n(stats["naki_rate"], *R[3]),
        n(100 - stats["houjuu_rate"], *R[4]),
        n(stats["avg_score_change"], *R[5]),
    ]
    av = [
        n(avg["ar"], *R[0]),
        n(avg["at"], *R[1]),
...
def render_cumulative_point_chart(games):
    ...
    for rank, color in rc.items():
        ...
        fig.add_trace(
            go.Scatter(
                x=df.loc[m, "game_number"],
                y=df.loc[m, "cumulative_point"],
                mode="markers",
                marker={"size": 10, "color": color},
                name=f"{rank}位",
                text=df.loc[m, "hover_text"],
...
def render_dan_chart(games):
    ...
    fig.add_trace(
        go.Scatter(
            ...
            marker={"size": 6, "color": COLORS["purple"]},
            text=df["dan_name"],
...
def render_score_donut(rounds, col, title, colors):
    ...
    counts = (
        pd.cut(data, bins=bins, labels=labels, right=False)
        .value_counts()
        .reindex(labels)
        .fillna(0)
    )
...
def render_round_score_bar(rounds):
    ...
    agg = (
        df.groupby(["round_number", "round_label"])
        .agg(
            avg_score=("score_change", "mean"),
            count=("score_change", "count"),
        )
        .reset_index()
        ...
        data=[
            go.Bar(
                ...
                marker_color=[
                    ...
                    for v in agg["avg_score"]
                ],
                text=[f"{v:+.0f}" for v in agg["avg_score"]],
                textposition="auto",
    ...
    fig = go.Figure(
        data=[
            go.Bar(
                ...
                text=df.apply(
                    lambda r: f"{int(r['count'])}回 ({r['pct']:.2f}%)", axis=1
                ),
                textposition="outside",
...
def render_trend_chart(rounds, games, period):
    ...
    fig = go.Figure()
    for col, color in [
        ("アガリ率", COLORS["positive"]),
        ("放銃率", COLORS["negative"]),
        ("リーチ率", COLORS["primary"]),
        ("副露率", COLORS["secondary"]),
    ]:
        fig.add_trace(
            go.Scatter(
                x=df["期間"],
                ...
                mode="lines+markers",
                line={"color": color},
                hovertemplate=f"{col}: %{{y:.2f}}%<br>局数: %{{customdata}}<extra></extra>",
                customdata=df["局数"],
            )
        )
    fig.update_layout(
...
def render_rank_trend(games, period):
    ...
    agg = (
        games.groupby("period")
        .agg(avg_rank=("final_rank", "mean"), games=("final_rank", "count"))
        .reset_index()
    )
    ...
    fig = go.Figure(
        data=[
            go.Bar(
                ...
                y=agg["avg_rank"],
                marker_color=[
                    COLORS["positive"] if v <= 2.5 else COLORS["negative"]
                    for v in agg["avg_rank"]
                ],
                text=[f"{v:.2f}" for v in agg["avg_rank"]],
                textposition="auto",
                hovertemplate="平均順位: %{y:.2f}<br>対局数: %{customdata}<extra></extra>",
        ...
        rows.append(
            {
                ...
                "アガリ回数": agari.shape[0],
                "平均打点": f"{int(agari['agari_ten'].mean()):,}"
                if not agari.empty
                else "-",
                "最高打点": f"{int(agari['agari_ten'].max()):,}"
                if not agari.empty
                else "-",
                "平均巡目": f"{agari['agari_turn'].mean():.1f}"
                if not agari.empty and agari["agari_turn"].notna().any()
                else "-",
                "放銃率": f"{houjuu.shape[0] / n * 100:.2f}%",
                "放銃回数": houjuu.shape[0],
                "放銃打点": f"{int(houjuu['houjuu_ten'].mean()):,}"
                if not houjuu.empty
                else "-",
...
def render_agari_type_chart(rounds: pd.DataFrame):
    ...
    for i, row in agg.iterrows():
        color = (
            type_colors[type_order.index(row["agari_type"])]
            if row["agari_type"] in type_order
            else COLORS["neutral"]
        )
        fig.add_trace(
            go.Bar(
...
def render_houjuu_type_chart(rounds: pd.DataFrame):
    ...
    for _, row in agg.iterrows():
        color = (
            type_colors[type_order.index(row["houjuu_to_type"])]
            if row["houjuu_to_type"] in type_order
            else COLORS["neutral"]
        )
        fig.add_trace(
            go.Bar(
                x=[row["houjuu_to_type"]],
                y=[row["count"]],
                name=row["houjuu_to_type"],
                marker_color=color,
                text=f"{row['pct']:.1f}%",
                textposition="auto",
                textfont={"size": 14},
    ...
    fig.update_layout(
        yaxis_title="回数", height=350, margin={"t": 30, "b": 50}, showlegend=False
    )
    return fig


def render_han_distribution(
    rounds: pd.DataFrame, col: str, title: str, colors: list[str]
):
    """翻数分布のドーナツチャート。"""
    data = rounds[rounds[col].notna()][col]
    if data.empty:
...
def render_outcome_waterfall(rounds: pd.DataFrame):
    ...
    outcomes = [
        ...
        (
            "ダマアガリ",
            rounds[rounds["is_agari"] & ~rounds["is_reach"] & ~rounds["is_naki"]],
        ),
    ...
    fig = go.Figure(
        go.Waterfall(
            ...
            y=values,
            measure=measures,
            increasing={"marker_color": COLORS["positive"]},
            decreasing={"marker_color": COLORS["negative"]},
            totals={"marker_color": COLORS["primary"]},
...
def render_opponents_situation_chart(
    rounds: pd.DataFrame, col: str = "opponents_reach_count", label: str = "他家リーチ"
):
    """他家状況別のアガリ率・放銃率チャート。"""
    data = []
...
def render_cross_analysis_heatmap(
    rounds: pd.DataFrame, row_axis: str, col_axis: str, metric: str
):
    ...
    if row_axis == "ALL":
        ...
        df["row_group"] = df.apply(
            lambda r: "リーチ" if r["is_reach"] else "副露" if r["is_naki"] else "ダマ",
            axis=1,
        )
        ...
        df["row_group"] = df.apply(
            lambda r: (
                "ツモ"
                if r.get("is_tsumo") is True
                else "出アガリ"
                if r.get("is_tsumo") is False
                else "非アガリ"
            ),
            axis=1,
        )
        ...
        df["row_group"] = (
            df["naki_count"]
            .clip(upper=3)
            .apply(lambda x: f"{x}回" if x < 3 else "3回以上")
        )
    ...
    if col_axis == "ALL":
        ...
        df["col_group"] = (
            df["opponents_reach_count"]
            .clip(upper=2)
            .apply(lambda x: f"{x}人" if x < 2 else "2人以上")
        )
    elif col_axis == "他家副露数":
        df["col_group"] = (
            df["opponents_naki_count"]
            .clip(upper=3)
            .apply(lambda x: f"{x}人" if x < 3 else "3人")
        )
    ...
    metric_fn = {
        ...
        "アガリ打点": lambda s: (
            s.loc[s["is_agari"], "agari_ten"].mean() if s["is_agari"].any() else None
        ),
    }
    ...
    text = [
        [
            f"{v:{fmt}}{suffix}<br>({c}局)" if v is not None else f"-<br>({c}局)"
            for v, c in zip(row_z, row_c)
        ]
        for row_z, row_c in zip(z, counts)
    ]
